...
int main(int argc, char **argv) {
  ...
  int bytes;
  while ((bytes = fread(send_buf, sizeof(byte), sizeof(send_buf), file_ptr)) >
         0) {
    cout << send_buf << endl;
    cout << bytes << endl;
  ...
  for (int i = 0; i < servers.size(); i++) {
    ...
    while ((extracted_string = strtok_r(NULL, "\n", &ptr)) != NULL) {
      ...
      for (int j = 0; j < dirs.size(); j++) {
        ...
        if (extractedFilename != dirs[j].name)
          continue;
...
void put(string filename, vector<server_t> &servers) {
  ...
  for (int i = 0; i < servers.size(); i++) {
    ...
    // Setting the ip dest.
    if (inet_pton(AF_INET, servers[server].ip.c_str(), &server_addr.sin_addr) <=
        0) {
      perror("Invalid address / Address not supported");
      close(sockfd1);
    }
    ...
    // Connect to server
    if (connect(sockfd1, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
        0) {
      perror("Connection failed");
      close(sockfd1);
    }
    ...
    // Setting the ip dest.
    if (inet_pton(AF_INET, servers[server2].ip.c_str(),
                  &server_addr2.sin_addr) <= 0) {
      perror("Invalid address / Address not supported");
      close(sockfd2);
    }

    // Connect to server
    if (connect(sockfd2, (struct sockaddr *)&server_addr2,
    ...
    while ((bytes_read = fread(send_buf, sizeof(char), bytes_to_read, file)) >
           0) {
      ...
      if (sizeof(send_buf) < curr_chunk_size) {
        ...
        if (i == servers.size() - 1)
          bytes_to_read = curr_chunk_size + (file_size % servers.size());
        else
          bytes_to_read = curr_chunk_size;
      }
    }
...
void get(string filename, vector<server_t> &servers) {
  ...
  for (int i = 0; i < servers.size(); i++) {
    ...
    for (int j = 0; j < servers.size(); j++) {
      ...
      // Setting the ip dest.
      if (inet_pton(AF_INET, servers[j].ip.c_str(), &server_addr.sin_addr) <=
          0) {
        perror("Invalid address / Address not supported");
        close(sockfd);
      }
    ...
    bzero(send_buf, sizeof(send_buf));
    while ((read_bytes = fread(send_buf, sizeof(char), sizeof(send_buf),
                               temp_file)) > 0) {
